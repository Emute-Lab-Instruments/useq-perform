...
1.5 **Eval highlight.** A flash decoration animates over the evaluated range on every eval. Soft eval uses a visually distinct flash to signal "preview, not committed." (See `src/editors/extensions/evalHighlight.ts`)
