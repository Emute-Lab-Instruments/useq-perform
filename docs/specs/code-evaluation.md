...
1.8 **Output assignment recognition.** When the evaluated form assigns to a recognised output (pattern `/\b([ads])([1-8])(?=[\s)(]|$)/g`), the app marks that output as "running" and **implicitly toggles vis on for that variant** so the user immediately sees what they evaluated. The implicit toggle is exclusive per output — it turns off any previously-toggled variant for the same output (full contract: [expression-gutter.md §3.4](expression-gutter.md)). Soft eval is the explicit exception and does **not** flip the toggle ([expression-gutter.md §3.4](expression-gutter.md)). (See `src/effects/editorEvaluation.ts`) The pattern is fixed and broad (a1–a8, d1–d8, s1–s8) — it covers the full theoretical range including future expanders. If a recognised output is not in the connected hardware's reported I/O config, the app still tracks it for WASM visualisation but flags a console warning. Recognition is syntactic and conservative — false negatives are acceptable, false positives are not.
