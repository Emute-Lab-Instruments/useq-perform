...
1.4 **Browser support floor**: a modern ES2020 browser that can run the bundle and the WASM module. Hardware mode additionally requires Web Serial (Chromium-family). When Web Serial is unavailable, the app stays usable in browser-local WASM mode rather than failing, but clearly alerts the user with a dismissable toast. (see `src/runtime/startupContext.ts` for `isWebSerialAvailable`)
