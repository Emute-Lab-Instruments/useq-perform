...
1.5 New entries animate in. Animation style is `console.entryAnimation` (`slide`, `fade`, `none`, default `slide`). Typewriter mode reveals text at `console.typewriterIntervalMs` (default 20ms) per character. A queue ensures only one entry animates at a time.
