...
- `src/ui/` — Solid components. Leaf layer; can import from anywhere.
  ...
  - `overlayManager.ts`.
  - `adapters/` — imperative mount bridges via `createSolidAdapter()` (toolbars, panels, modal, picker-menu, double-radial-menu, snippets, settings, visualisation, palette, modifier-hints, visualisation-panel, `mainMenu.tsx` for the main menu overlay, `liveEditPanel.tsx` for the dockable live-edit panel, `calibration.tsx` for the full-screen calibration takeover).
