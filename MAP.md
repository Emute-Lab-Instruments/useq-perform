...
- `harness/` — shared editor + extension scenario harness used by Storybook.
...
- `src/lib/` — foundation. No imports from runtime/effects/ui/editors/transport.
  - `settings/` — schema, normalization, persistence (split out of the old `appSettings.ts`). Section-owned normalization lives in `normalizeEvalResults.ts`, `normalizeKeybindings.ts`, and `normalizeVisualisation.ts`; shared coercion is in `normalizationHelpers.ts`, while import validation/diff lives in `configurationValidation.ts`. Includes `HardwareSettings` (bindingsEnabled, bindingFoldDefault, bindingQueueDepth, holdTickHz) per [docs/specs/hardware-bindings.md §6](docs/specs/hardware-bindings.md). Includes `FormatSettings` (lineWidth, complexityThreshold, minAvailableWidth, indentStyle, autoFormatOnMutation) per [docs/specs/formatting.md §5](docs/specs/formatting.md). Includes `LiveEditSettings` (idAlphabet/idLength + §10 surface) per [docs/specs/live-edit.md §10](docs/specs/live-edit.md) and `CalibrationSettings` (sliderRangeCents, snapZeroToleranceCents, fine/coarseStepCents, carryForwardOffset, …) per [docs/specs/calibration.md §9](docs/specs/calibration.md).
  - `keybindings/` — pure action registry (with `reversible` flag, derives `ReversibleActionId`/`NonReversibleActionId`), `defaults.ts` (default binding data), resolver (merges profile defaults + overrides and receives higher-layer handlers through injection), `profileRegistry.ts` (profile id → base binding set), `contexts.ts` (`when`-predicate registry/evaluator), `layouts/` (qwerty-us/uk, dvorak, colemak, azerty-fr, qwertz-de), `profiles/` (including `simplified.ts`), sticky modifiers, and the OS-reserved key list. See [docs/specs/keybindings.md](docs/specs/keybindings.md).
  - `editorStore.ts`, `editorDefaults.ts`, `editorCompartments.ts` — active `DocumentSession` signal/view facade, small session-routed editor operations, and foundation-level CodeMirror configuration. Editor construction and ownership belong to `src/editors/documentSession.ts` + `editorLifecycle.ts`.
  - `typedChannel.ts` — pub/sub primitive used by everything in `contracts/`.
  - `persistence.ts`, `documentRecord.ts` — central localStorage primitives plus the versioned atomic text+identity `DocumentRecord`; split code/identity keys are migration inputs, not live authorities.
  ...
  - `menu/` — **radial menu system** (spec: `docs/specs/radial-menu.md`). Gamepad-driven double-ring command surface for noun insertion. `dispatcher.ts` is the thin lifecycle/action adapter; `textEntry.ts` owns numpad/T9 layouts, hover and multi-tap timing; `verbApplication.ts` resolves selections and applies structural verbs; `editorTarget.ts` owns the CodeMirror/structural-tree boundary; `chainCoordination.ts` plans close/reopen inputs over the pure `chain.ts` runner. `state.ts` owns reducer semantics, `verbs.ts` owns verb transformations, `manifest.ts` + `manifest.json` own ring content, and `store.ts` bridges state to Solid. Heavily tested by focused state, dispatcher/e2e, verb, chain, text-entry, manifest, property and performance suites.
...
- `src/contracts/` — shared types/constants and typed channel definitions. See [docs/specs/reactive-flow.md](docs/specs/reactive-flow.md) for channel inventory.
  - `useqProtocolSchema.generated.ts`, `useqProtocolSchema.ts` — generated JSON-v1 catalog plus shared request/response/unsolicited validators and derived hardware/WASM inventories; source is `src-useq/protocol/useq-json-v1.json`.
  - `runtimeChannels.ts`, `visualisationChannels.ts`, `gamepadChannels.ts`, `hardwareChannels.ts`, `editorChannels.ts`, `guideChannels.ts` — channel registries. `gamepadChannels.ts` contains only the continuous manual-control stick axis; discrete gamepad input resolves to `ActionId`. `guideChannels.ts` carries user-guide navigation requests, so a diagnostic rendered by `src/editors/` can deep-link into the guide without that layer importing `src/ui/` ([docs/specs/the-machine.md](docs/specs/the-machine.md) §5.1). `hardwareChannels.ts` carries discrete hardware input events (button presses, toggle flips, gate edges). `editorChannels.ts` carries structural navigation/mutation events consumed by radial menu and keyboard hints.
  ...
  - `runtimeTypes.ts`, `visualisationEvents.ts` — runtime and visualisation payload types; `runtimeTypes.ts` also owns exhaustive, reason-bearing synth-artifact validation before audio commit.
  ...
  - `synthesisControlAbi.ts` — stable public barrel and single named-export home for the versioned SharedArrayBuffer transport plus engine constants (`SYNTH_FADE_IN/OUT_MS`, `CONTROL_LOOKAHEAD_BLOCKS`, `PRODUCER_TIMEOUT_BLOCKS`, `MAX_SYNTH_*`, …). Internals: `synthesisControlAbi/layout.ts` exclusively defines binary layout/constants and buffer validation, `view.ts` owns typed non-blocking ring access, and `pacing.ts` owns the Worker-only bounded waiter and static invariants. See [docs/specs/synthesis.md §4](docs/specs/synthesis.md).
...
- `src/audio/` — browser synthesis engine (M0–M2.2 of [docs/design/synthesis-epic.md](docs/design/synthesis-epic.md); spec [docs/specs/synthesis.md](docs/specs/synthesis.md)).
  - `synthesisService.ts` — main-thread session actor: explicit prepare/commit/recover/teardown transactions, phase-safe producer bridge, serial failure-atomic graph activation (incl. `MAX_SYNTH_NODES` + block-rate channel-pool checks), telemetry. Accessed via `src/runtime/activeSynthesisService.ts`.
  ...
  - `testing/synthesisServiceFakes.ts` — composable AudioContext, worklet, NodeDef-loader, capability, and telemetry fixtures shared by service contract suites; scenario-specific failure controls remain local to each suite.
  ...
  - `workletCore.ts` + `synthesisWorklet.ts` — AudioWorkletProcessor logic (framework-free core + thin worklet shell with explicit NodeDef-install failure acknowledgements); multi-node topological graph host with transaction-owned candidate zones, acknowledged block-boundary activation, retryable failure-atomic NodeDef blocks (preallocated state rollback + whole-zone mute), sample-indexed partition-invariant fades, and binary64 terminal mixing with one final binary32 conversion (synthesis.md §3.1/§3.3/§3.5.1/§3.6); bundled by `scripts/build-assets.mjs` into `public/wasm/synthesisWorklet.js`.
  - `workletTransitionPlanning.ts` — pure stable graph-order/terminal planning and allocation-free producer-liveness transition decisions consumed by `workletCore.ts`.
  ...
  - `workletGraphDelta.ts`, `nodeDefAdapter.ts` — graph delta application and source-agnostic NodeDef module adapter, including worklet-safe strict UTF-8 registry decoding, module-metadata validation, named audio-input execution, and the versioned actual-sample-rate compute capability.
...
- `src/transport/` — Web Serial lifecycle and protocol. No UI/editor imports.
  ...
  - `json-protocol.ts` — capability negotiation plus the protocol-v1 JSON driver (handshake, heartbeat, eval), validating through the generated schema catalog. See [src-useq/docs/specs/wire-protocol.md](src-useq/docs/specs/wire-protocol.md).
...
- `src/runtime/` — bootstrap, lifecycle, runtime services. May import UI only from `bootstrap.ts` and `appLifecycle.ts` (eslint exceptions).
  ...
  - `browserWasmRuntime.ts` — Worker lifecycle module: configured intent, actual availability publication, one controlled crash replacement, ABI/load failure reporting, and disposal.
  - `bootstrapRecoverySurface.ts` — framework-independent recovery notice used when Solid or the Worker runtime cannot start.
  ...
  - `runtimeSession.ts` — pure hardware-vs-WASM precedence and mode derivation; consumers read or subscribe to session state through the coordinator/runtime services.
  ...
  - `wasmInterpreter.ts` — non-production direct WASM loader used by isolated witnesses and interpreter integration tests; production never imports it.
  ...
  - `wasmRuntimeWorkerPort.ts` — sole production `WasmRuntimePort`; proxies every method to a dedicated classic Worker, preserves typed ABI errors, reports post-ready crashes, and owns termination/in-flight rejection.
  ...
  - `browserEvalSurface.ts` — devmode `window.__useqBrowserEval` surface: verified eval route + `sampleOutputAtTime` + deterministic clock hooks (`freezeClock`/`stepClock`/`resumeClock`/`isClockFrozen`) over the visualisation session's clock seam; used by the first-sound listening guide ([docs/synthesis/LISTENING_GUIDE.md](docs/synthesis/LISTENING_GUIDE.md)) and the e2e journeys.
...
- `src/effects/` — composable side-effect modules. Framework-agnostic where possible.
  ...
  - `visualisationSession.ts` — sole production visualisation seam, exposing clock, expression, view-data, probes, asymmetric hardware→WASM shadow, and one dispose lifetime. The modules below it are implementation details.
  ...
  - `visualisationProjectionPlan.ts`, `visualisationBufferApplication.ts`, `visualisationProjectionTrace.ts`, `visualisationSampler.ts` — pure projection decision table, finite-prefix application policy, trace summaries, and WASM sampling/projection executor. `visualisationBuffers.ts` exclusively owns past/future buffer allocation, capacity, and render lookup; `visualisationSamplingPolicy.ts` applies the live loop's bounded settings policy and projection identity after general app-settings normalization. The sampler retains expression lifecycle and projection scheduling.
  ...
  - `calibrationSequencer.ts` — aspirational CV 1V/oct calibration session state machine (begin/adjust/save-point/end lifecycle per [docs/specs/calibration.md](docs/specs/calibration.md)); the pinned firmware currently rejects the reserved message family until its takeover backend lands.
...
- `src/editors/` — CodeMirror layer. Imports lib/contracts/effects/transport.
  - `documentSession.ts` — text-canonical per-editor ownership seam: coherent CodeMirror revision snapshots, session-local identity field, atomic autosave/flush, subscriptions, and teardown. Main is durable; guide/theme/Zen/example sessions are isolated and ephemeral.
  - `editorLifecycle.ts` — durable main-session construction, runtime persistence-policy subscription, theme setup, and application-lifetime disposal; secondary UI surfaces create explicit ephemeral sessions at their owning component lifetime.
  - `commands/actionHandlers.ts` — source-aware `ActionId` → editor command/runtime/UI mapping; production injects it into the foundation resolver and gamepad pipeline.
  - `commands/editorCommandRouter.ts` — typed editor-intent vocabulary and sole commit/focus policy for app-authored keyboard, gamepad, menu, palette, widget, and test commands; structural action names are checked against the adapter catalog.
  ...
  - `extensions/probes.ts`, `extensions/probes/`, `probeHelpers.ts` — thin CodeMirror probe adapter over separated persistence/model, sampling/highlight, and WebGL/DOM rendering modules (DI-configured).
  ...
  - `extensions/liveEdit/` — live-edit editor layer. `widgets.ts` owns CodeMirror state/decorations, while `widgetBase.ts`, `numericWidgets.ts`, `choiceWidgets.ts`, and `widgetTheme.ts` partition shared, numeric, choice, and styling behavior (spec §4). `vectorMarking.ts` owns vector-mark rendering (spec §3.7); `markAction.ts` owns mark/unmark/commit/reset/rename/editRange and insertion mode (spec §3/§6); `rangeInference.ts`, `vectorMarkController.ts`, `widgetStoreBridge.ts`, `pasteHandler.ts`, and `idleEval.ts` own their named boundaries. Tests live in `__tests__/`.
  ...
  - `keymaps.ts`, `editorKeyboard.ts`, `themes.ts`; `gamepadNavigation.ts` owns only the editor-context reader, continuous manual-control axis application, and its disposable DOM/channel subscriptions.
- `src/ui/` — Solid components. Leaf layer; can import from anywhere.
  - `ApplicationRoot.tsx` — the main route's single Solid render owner; portals preserve the static toolbar mount-point contract and application shutdown disposes the whole reactive lifetime.
  ...
  - `overlayManager.ts`.
  - `adapters/` — wired, application-owned component trees and imperative state APIs for panels, modal/menu/palette overlays, toolbars, virtual gamepad, live-edit, calibration, and visualisation visibility; they do not create independent Solid roots. `engineIndicatorRecovery.ts` owns the tested recover-then-resume callback.
  ...
  - `calibration/` — full-screen aspirational CV 1V/oct calibration takeover (`CalibrationTakeover.tsx`, `CalibrationPicker.tsx`, `CalibrationSlider.tsx`, `CalibrationProgress.tsx`, `CalibrationCompleteBanner.tsx`) per [docs/specs/calibration.md](docs/specs/calibration.md). UI shell only; current firmware support is explicitly absent.
  ...
  - `visualisation/` — `serialVisGL.ts` (WebGL2 canvas/program/VBO owner and paint coordinator), `serialVisPlanning.ts` (DOM-free lane layout, past/future sample assembly, and pixel-matched rate planning), `webglLineRenderer.ts` (low-level line geometry/shader helpers). Pure entry `drawSerialVisGL(input)` supports harnesses; wired entry `drawSerialVisGLFromStores()` serves production. Render hooks and view data cross through `effects/visualisationSession.ts`; sampling/buffers/store remain internal implementation modules.
...
- `scripts/check-doc-source-paths.mjs` — fails when trusted orientation docs point at deleted source files; runs as part of `npm run lint`.
...
- `package.json` — see scripts: `dev`, `build`, `build:assets`, `build:wasm`, `harvest:witnesses`, `harvest:specs`, `test:mocha`, `test:unit`, `test:all`, `test:e2e`, `test:browser-local-eval`, `test:release`, `lint`, `typecheck`, `storybook`, `build-storybook`, `src-useq:status`.
...
- Main-document text is authoritative in its CodeMirror `EditorState`; **`DocumentSession`** atomically persists text plus identity metadata. `editor.code` and the old split keys are bootstrap/migration inputs only.
...
- CodeMirror extensions that need runtime globals use a **Config interface + factory** pattern (e.g. `ProbeConfig` / `createProbeExtensions()`), with a `createDefaultXxxConfig()` wiring app singletons. Lets extensions render in Storybook in isolation.
...
- [docs/specs/storybook.md](docs/specs/storybook.md) — canonical isolated visual review surface and browser gate.
...
- [docs/synthesis/LISTENING_GUIDE.md](docs/synthesis/LISTENING_GUIDE.md) — user-run first-sound manual validation procedure; current-browser scheduling and non-zero finite samples are automated, while physical routing and perceptual audibility remain manual.
