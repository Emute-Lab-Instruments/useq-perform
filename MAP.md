...
- `src/effects/` — composable side-effect modules. Framework-agnostic where possible.
  ...
  - `visualisationProjectionPlan.ts`, `visualisationBufferApplication.ts`, `visualisationProjectionTrace.ts`, `visualisationSampler.ts` — pure projection decision table, finite-prefix buffer policy, trace summaries, and WASM executor. The sampler owns buffers and expression lifecycle; the planner decides reset-fill, extend, skip, or tick-only without side effects.
