...
- `src/lib/` — foundation. No imports from runtime/effects/ui/editors/transport.
  ...
  - `editorStore.ts`, `editorDefaults.ts`, `editorCompartments.ts` — active-view signal, small view operations, and foundation-level CodeMirror configuration. Editor construction and lifecycle belong to `src/editors/editorLifecycle.ts`.
...
- `src/editors/` — CodeMirror layer. Imports lib/contracts/effects/transport.
  - `editorLifecycle.ts` — construction, runtime settings subscription, autosave timer, theme setup, and destruction for the main/example editors.
