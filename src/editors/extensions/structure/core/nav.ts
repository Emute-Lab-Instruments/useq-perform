 ...
 * (`Meta.payload: unknown`, see `types.ts`) and are not represented as
 * addressable Nodes, so there is nothing for a cursor to land on. Implementing
 * it requires `treeFromLezer` to lower structured Meta payloads (metadata
 * value, wrapper keyword-args) into real, id-bearing Nodes first (tracked
 * alongside the wrapper-recognition work). Until then the op is absent from
 * `nav` rather than a no-op stub.
 */
