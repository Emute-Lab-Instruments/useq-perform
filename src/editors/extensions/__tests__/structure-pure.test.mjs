...
// Map pattern to palette index (simplified for testing)
function getMatchColor(match) {
    const [full, type, digit] = match;
    const colors = ['red', 'green', 'blue', 'yellow', 'purple'];
    if (type === "a") {
        if (digit === "1") return colors[0];
        if (digit === "2") return colors[1]; 
        if (digit === "3") return colors[2];
    }
    if (type === "d") {
        if (digit === "1") return colors[2];
        if (digit === "2") return colors[3];
        if (digit === "3") return colors[4];
    }
    return colors[0];
}

// Pure function: Find all expression ranges in document text
function findExpressionRanges(docLines, findBoundsFn) {
...
function isRangeActive(range, lastEvaluated) {
...
// Mock marker class for testing
class MockExpressionGutterMarker {
    constructor(color, isStart, isEnd, isMid, isActive) {
        this.color = color;
...
function createMarkersForRange(range, isActive, docLineFn) {
    const markers = [];
    ...
    for (let line = range.from; line <= range.to; line++) {
        ...
        const marker = new MockExpressionGutterMarker(range.color, isStart, isEnd, isMid, isActive);
...
function processExpressionRanges(expressionRanges, lastEvaluatedMap, docLineFn) {
    const allMarkers = [];
...
function findExpressionAtPosition(cursor, lineText, lineFrom, findBoundsFn) {
    let match;
...
// Test 5: findExpressionRanges should parse document correctly
function testFindExpressionRanges() {
    console.log('Testing findExpressionRanges...');
    
    const docLines = [
        { text: '(a1 first)', from: 0 },
        { text: '(a1 second)', from: 11 },
        { text: '(d2 third)', from: 23 }
    ];
    
    const ranges = findExpressionRanges(docLines, mockFindBoundsFn);
    
    console.assert(ranges.has('a1'), 'Should find a1 expressions');
    console.assert(ranges.has('d2'), 'Should find d2 expressions');
    console.assert(ranges.get('a1').length === 2, 'Should find 2 a1 expressions');
    console.assert(ranges.get('d2').length === 1, 'Should find 1 d2 expression');
    
    const a1Ranges = ranges.get('a1');
    console.assert(a1Ranges[0].color === 'red', 'First a1 should have correct color');
    console.assert(a1Ranges[1].color === 'red', 'Second a1 should have correct color');
    
    const d2Ranges = ranges.get('d2');
    console.assert(d2Ranges[0].color === 'yellow', 'D2 should have correct color');
    
    console.log('✓ findExpressionRanges tests passed');
}
