...
// ---------------------------------------------------------------------------
// applySynthArtifacts backwards-compat (VAL-COMP-013/014/015)
// ---------------------------------------------------------------------------

describe("applySynthArtifacts — backwards-compat wrapper", () => {
  beforeEach(() => {
    resetEngineStateStoreForTests();
  });

  it("returns true for a committed payload", async () => {
    const bundle = buildBundle();
    const service = createSynthesisService(bundle.options);
    await resumeService(service);

    // Import via dynamic to avoid circular deps at module load.
    const { applySynthArtifacts } = await import("./synthesisService");
  ...
  it("returns false for a failed-eval payload", async () => {
    const bundle = buildBundle();
    const service = createSynthesisService(bundle.options);
    await resumeService(service);
