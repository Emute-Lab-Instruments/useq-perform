...
import { enterBootloaderMode } from '../io/serialComms.ts';

// --- DOM helper utilities ---

function createEl(tag: string, opts: any = {}): HTMLElement {
...
function fadeOut(el: HTMLElement, duration = 300, onDone?: () => void) {
...
function markStepComplete(panel, stepNumber) {
    ...
    if (stepNum) {
        (stepNum as HTMLElement).style.backgroundColor = 'var(--accent-color)';
        (stepNum as HTMLElement).style.color = '#000';
    }

    const stepStatus = step.querySelector('.step-status');
    if (stepStatus) {
        (stepStatus as HTMLElement).style.opacity = '1';
    }
}
...
function showUpdateComplete(panel) {
    ...
    if (completion) {
        fadeIn(completion as HTMLElement);
    }
}
...
if (typeof window !== 'undefined') {
    (window as any).downloadLatestUF2 = downloadLatestUF2;
    (window as any).startUpgradeFlow = startUpgradeFlow;
}
