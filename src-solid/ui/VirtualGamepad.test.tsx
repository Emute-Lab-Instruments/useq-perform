...
describe("VirtualGamepad", () => {
  ...
  describe("window.__virtualGamepad interop", () => {
    it("exposes get/set/reset on window.__virtualGamepad", () => {
    ...
    it("set updates state through window API", () => {
      window.__virtualGamepad!.set({ axes: [0.1, 0.2, 0.3, 0.4] });
      const state = window.__virtualGamepad!.get();
      expect(state.axes).toEqual([0.1, 0.2, 0.3, 0.4]);
    });

    it("reset restores defaults through window API", () => {
